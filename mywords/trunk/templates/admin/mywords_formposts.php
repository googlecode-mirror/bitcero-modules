...
<div class="outer">
	...
	<div class="mw_bcontent">
		<table class="outer<?php echo !$edit || (!isset($post) && !$post->fields()) ? ' mw_hidden' : ''; ?>" cellspacing="0" />
			<tr class="head">
			...
			<?php foreach($post->fields() as $field): ?>
				<tr class="<?php echo cycle("even,odd"); ?>">
					<td><input type="text" name="meta[<?php echo $field->id(); ?>][key]" id="meta[<?php echo $field->id(); ?>][key]" value="<?php echo $field->name(); ?>" class="mw_large" /></td>
		...
		<table class="outer" cellspacing="0" />
			...
			<tr class="even">
				<td valign="top">
					...
					<select name="meta_name_sel" class="mw_large">
						<option value="" selected="selected"><?php _e('- Select -','admin_mywords'); ?></option>
						<?php foreach ($meta_names as $name): ?>
						<option value="<?php echo $name; ?>"><?php echo $name; ?></option>
						<?php endforeach; ?>
					</select>
					<input type="text" name="meta_name" id="meta-name" value="" class="mw_large" style="display: none;" />
			...
			<tr class="odd">
				<td colspan="2">
					<input type="button" class="mw_addfield" value="<?php _e('Add custom field','admin_mywords'); ?>" />
				</td>
			</tr>
		</table>
