...
function xt_install_theme(){
    ...
    if (!is_file($pdir.'/config/theme.php'))
    	redirect_header('index.php', 1, sprintf(__('Theme "%s" does not exists!','xthemes'), $dir));
    
    if (false===($theme = xt_is_valid($dir)))
   		redirect_header('index.php', 1, sprintf(__('Theme "%s" is not a valid XThemes theme!','xthemes'), $dir));
    ...
    if (method_exists($theme, 'check_requirements')){
    	if (!$theme->check_requirements())
    		redirect_header('index.php', 1, sprintf(__('Theme "%s" return next errors:', 'xthemes'), $theme->name()).'<br /><br />'.implode("<br />", $theme->errors()));
	}

    // Insert theme data
    if (!$db->queryF("UPDATE ".$db->prefix("config")." SET `conf_value`='$dir' WHERE `conf_name`='theme_set' AND conf_modid='0'"))
        redirect_header('index.php', 1, __('The theme has not been installed!','xthemes').'<br />'.$db->error());
    
    $db->queryF("UPDATE ".$db->prefix("config")." SET `conf_value`='".serialize(array('glass'))."' WHERE `conf_name`='theme_set_allowed' AND conf_modid='0'");
    ...
    redirect_header('index.php', 1, __('Theme installed successfully','xthemes').(count($messages)>0 ? " ".__("but there was some errors:",'xthemes')."<br />".implode("<br />".$messages) : ''));
}
...
function xt_install_normal(){
    ...
    if (is_file($pdir.'/config/theme.php'))
    	redirect_header('index.php', 1, sprintf(__('¡"%s" is a xThemes theme!','xthemes'), $dir));
