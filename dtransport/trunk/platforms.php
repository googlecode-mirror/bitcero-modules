...
if($mc['inner_dest_download']){
    $xoopsTpl->assign('featured_items', $dtfunc->items_by(array($os->id()), 'platforms', 0, 'featured', 0, $mc['limit_destdown']));
...
if($mc['inner_daydownload']){
    $xoopsTpl->assign('daily_items', $dtfunc->items_by($os->id(), 'platforms', 0, 'daily', 0, $mc['limit_daydownload']));
...
$xoopsTpl->assign('lang_in', sprintf(__('<strong>Downloads</strong> for "%s"', 'dtransport'), $os->name()));
