<?php
// $Id: dtsoftware.class.php 28 2008-02-14 23:56:22Z ginis $
// --------------------------------------------------------------
// D-Transport
...
class DTSoftwareEdited extends EXMObject
{
	private $_tags=array();
	...
	public function software(){
		return $this->getVar('soft');
		
	}
		
	public function setSoftware($id){
		
		return $this->setVar('soft',$id);
	...
	**/	
	public function fileGroups($asobj = false){
		...
		if (empty($this->_groups) || ($asobj && !is_a($this->_groups[0], 'DTFileGroup'))){
			$this->_groups = array();
			$sql="SELECT * FROM ".$this->db->prefix('dtrans_groups')." WHERE id_soft=".$this->id();
			$result=$this->db->queryF($sql);
			while ($rows=$this->db->fetchArray($result)){
				if ($asobj){
					$tmp = new DTFileGroup();
	...
	**/
	...
	/**
	* @desc Obtiene el arreglo fields
	**/
	...
	/**
	* @desc Almacena las etiquetas, alerta, licencias y plataformas 
	* en un arreglo cuando una edición no ha sido aprobada
	**/
	public function setFields(){
		
		$this->_fields['tags'] = $this->_tags;
		$this->_fields['licences'] = $this->licences;
		$this->_fields['platforms'] = $this->_platforms;
		if ($this->activealert){
			$limit = $this->_alert->limit();
			$mode = $this->_alert->mode();
		}else{
			$limit = 0;
			$mode = 0;
		}		

		$this->_fields['alert']= array('active'=>$this->activealert,'limit'=>$limit,'mode'=>$mode);
		
		return $this->setVar('fields',$this->_fields);
