...
function rmmf_bk_recent_edit($options){
	$form = _BK_RMMF_NUMWORKS."<br /><input type='text' size='5' name='options[]' value='$options[0]' />";
	return $form;
}
...
function rmmf_bk_featured($options){
	...
	$result = $db->query("SELECT * FROM ".$db->prefix("rmmf_works")." WHERE resaltado='1' ORDER BY id_w DESC LIMIT 0,$options[0]");
	$block = array();
	while ($row = $db->fetchArray($result)){
		$rtn = array();
		$rtn['id'] = $row['id_w'];
...
function rmmf_bk_featured_edit($options){
	$form = _BK_RMMF_NUMWORKS."<br /><input type='text' size='5' name='options[]' value='$options[0]' />";
	return $form;
}
...
function rmmf_bk_random($options){
	...
	$result = $db->query("SELECT * FROM ".$db->prefix("rmmf_works")." ORDER BY RAND() LIMIT 0,$options[0]");
	$block = array();
	while ($row = $db->fetchArray($result)){
		$rtn = array();
		$rtn['id'] = $row['id_w'];
...
function rmmf_bk_random_edit($options){
	$form = _BK_RMMF_NUMWORKS."<br /><input type='text' size='5' name='options[]' value='$options[0]' />";
	return $form;
}
?>