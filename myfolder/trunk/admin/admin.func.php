...
function rmmf_make_adminnav(){
	...
	echo "<table width='100%' class='outer' cellspacing='1'>
			...
			<td style='".((_RMMF_LOCATION=='INDEX') ? "background: url(../images/bgmenuselec.jpg) repeat-x; " : "")."vertical-align: middle; border-right: 1px solid #CCCCCC; border-bottom: 1px solid #CCCCCC;' onmouseover=\"this.style.background='url(../images/bgmenu1.jpg) repeat-x;';\" onmouseout=\"this.style.background='url(../images/".((_RMMF_LOCATION=='INDEX') ? "bgmenuselec.jpg" : "bgmenu.jpg").") repeat-x;';\">
			<a href='./'>"._MA_RMMF_WORKS."</a></td>
			<td style='".((_RMMF_LOCATION=='NEWWORK') ? "background: url(../images/bgmenuselec.jpg) repeat-x; " : "")."vertical-align: middle; border-right: 1px solid #CCCCCC; border-bottom: 1px solid #CCCCCC;' onmouseover=\"this.style.background='url(../images/bgmenu1.jpg) repeat-x;';\" onmouseout=\"this.style.background='url(../images/".((_RMMF_LOCATION=='NEWWORK') ? "bgmenuselec.jpg" : "bgmenu.jpg").") repeat-x;';\">
			<a href='./?op=new'>"._MA_RMMF_NEWWORK."</a></td>
			<td style='".((_RMMF_LOCATION=='CATEGOS') ? "background: url(../images/bgmenuselec.jpg) repeat-x; " : "")."vertical-align: middle; border-right: 1px solid #CCCCCC; border-bottom: 1px solid #CCCCCC;' onmouseover=\"this.style.background='url(../images/bgmenu1.jpg) repeat-x;';\" onmouseout=\"this.style.background='url(../images/".((_RMMF_LOCATION=='CATEGOS') ? "bgmenuselec.jpg" : "bgmenu.jpg").") repeat-x;';\">
			<a href='categos.php'>"._MA_RMMF_CATEGOS."</a></td>
			<td style='".((_RMMF_LOCATION=='NEWCATEGO') ? "background: url(../images/bgmenuselec.jpg) repeat-x; " : "")."vertical-align: middle; border-right: 1px solid #CCCCCC; border-bottom: 1px solid #CCCCCC;' onmouseover=\"this.style.background='url(../images/bgmenu1.jpg) repeat-x;';\" onmouseout=\"this.style.background='url(../images/".((_RMMF_LOCATION=='NEWCATEGO') ? "bgmenuselec.jpg" : "bgmenu.jpg").") repeat-x;';\">
			<a href='categos.php?op=new'>"._MA_RMMF_NEWCATEGO."</a></td>
			</tr></table><br />";
}
...
function rmmf_make_footer($echo = true){
	...
	$rtn .= "Powered by <a href='http://www.redmexico.com.mx'>RMSOFT MyFolder 1.2</a> &nbsp;|
			&nbsp;Visit <a href='http://www.xoops-mexico.net'>XOOPS-MEXICO.NET</a>.
			</div>";
 ...
 */
function rmmf_make_random($size=8, $prefix=''){
 ...
 */
function rmmf_web_dir($dir){
	
	$dir = rmmf_add_slash($dir);
...
function resize_then_crop( $filein,$fileout,$imagethumbsize_w,$imagethumbsize_h,$red,$green,$blue)
{
	...
	switch($format){
    	case 'image/jpeg':
        	imagejpeg($thumb2, $fileout, 90);
           	break;
        case 'image/gif';
           	imagegif($thumb2, $fileout);
           	break;
        case 'image/png':
           imagepng($thumb2, $fileout, 90);
           	break;
	} //write to file
	//header('Content-type: image/gif');
...
function rmmf_localize($id, $by){
	...
	$ret = '';
	if ($by==0){
		$result = $db->query("SELECT id_cat, nombre, parent FROM ".$db->prefix("rmmf_categos")." WHERE id_cat='$id'");
		if ($db->getRowsNum($result)<=0){ return; }
		$row = $db->fetchArray($result);
		if ($row['parent']>0){ $ret .= rmmf_localize($row['parent'], 0); }
		$ret .= " &raquo; <a href='categos.php?id=$id'>$row[nombre]</a>";
	} else {
		$result = $db->query("SELECT id_w, titulo, catego FROM ".$db->prefix("rmmf_works")." WHERE id_w='$id'");
		if ($db->getRowsNum($result)<=0){ return; }
		$row = $db->fetchArray($result);
		$ret .= rmmf_localize($row['catego'], 0);
		$ret .= " &raquo; <a href='view.php?id=$id'>$row[titulo]</a>";
	}
