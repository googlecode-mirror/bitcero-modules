<?php
// $Id: text.class.php 825 2011-12-09 00:06:11Z i.bitcero $
// --------------------------------------------------------------
...
class RMFormText extends RMFormElement
{
	...
	public function render(){
		$ret = '<input type="'.($this->_password ? 'password' :'text').'" size="'.$this->_size.'" name="'.$this->getName().'" id="'.$this->getName().'"'.($this->getMax()>0 ? ' maxlength="'.$this->getMax().'"' : '').' value="'.(isset($_REQUEST[$this->getName()]) ? $_REQUEST[$this->getName()] : $this->getValue()).'" ';
		if ($this->getClass() != ''){
