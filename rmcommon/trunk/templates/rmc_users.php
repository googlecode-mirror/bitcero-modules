...
<table class="outer" cellspacing="0" width="100%">
    <thead>
    <tr>
        <th width="20"><input type="checkbox" class="checkall" id="checkall-top" onclick="$('#form-users').toggleCheckboxes(':not(#checkall-top)');" /></th>
        <th align="center"><?php _e('ID','rmcommon'); ?></th>
        <th align="left"><?php _e('Username','rmcommon'); ?></th>
        <th align="left"><?php _e('Name','rmcommon'); ?></th>
        <th><?php _e('Email','rmcommon'); ?></th>
        <th><?php _e('Groups','rmcommon'); ?></th>
        <th><?php _e('Status','rmcommon'); ?></th>
    </tr>
    </thead>
    <tfoot>
    <tr>
        <th width="20"><input type="checkbox" id="checkall-bottom" name="checkall" onclick="$('#form-users').toggleCheckboxes(':not(#checkall-bottom)');" /></th>
        <th align="center"><?php _e('ID','rmcommon'); ?></th>
        <th align="left"><?php _e('Username','rmcommon'); ?></th>
        <th align="left"><?php _e('Name','rmcommon'); ?></th>
        <th><?php _e('Email','rmcommon'); ?></th>
    ...
    <tr class="even">
    	<td colspan="6" align="center" style="color: #F00;">
    		<?php _e('There are not any user registered with for this filter.','rmcommon'); ?>
    	</td>
    </tr>
    <?php endif; ?> 
    <?php 
    ...
    $qstring = '';
    foreach (RMTemplate::get()->vars() as $var => $value){
    ...
    <tr class="<?php echo tpl_cycle('even,odd'); ?><?php echo $user['level']<=0 ? ' user_inactive' : '' ?>" valign="top">
        ...
        <td align="center"><a href="javascript:;" title="<?php echo sprintf(__('Send email to %s','rmcommon'), $user['uname']); ?>"><?php echo $user['email']; ?></a></td>
        <td align="center" class="users_cell_groups">
        	<?php 
        		$str = '';
