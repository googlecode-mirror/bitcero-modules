...
<div id="rmc-container">
    ...
    <div id="rmc-left-widgets">
        ...
        <?php $kid = TextCleaner::getInstance()->sweetstring($widget['title']); ?>
        <div class="rmc_widget_title<?php echo isset($widv[$kid]) && $widv[$kid]=='false' ? ' title-collapsed' : '';?><?php if($k==count($left_widgets)-1): ?> last-widget<?php endif; ?>" id="wid-title-<?php echo $kid; ?>"><span<?php echo isset($widget['icon']) && $widget['icon']!='' ? ' style="background-image: url('.$widget['icon'].'); padding-left: 26px;"' : ''; ?>><?php echo $widget['title']; ?></span></div>
        <div class="rmc_widget_content" id="wid-content-<?php echo $kid; ?>"<?php echo isset($widv[$kid]) && $widv[$kid]=='false' ? ' style="display: none;"' : ''; ?>><?php echo $widget['content']; ?></div>
        <?php endforeach; ?>
        </div>
    </div>
    <?php endif; ?>
